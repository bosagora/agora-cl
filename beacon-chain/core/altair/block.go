...
func ApplySyncRewardsPenalties(ctx context.Context, s state.BeaconStateAltair, votedIndices, didntVoteIndices []types.ValidatorIndex) (state.BeaconStateAltair, error) {
	...
	proposerReward, participantReward, err := SyncRewards(activeBalance, s)
	if err != nil {
		return nil, err
	}
...
func SyncRewards(activeBalance uint64, s state.BeaconStateAltair) (proposerReward, participantReward uint64, err error) {
